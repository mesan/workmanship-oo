...
public class AccountUtil {
    ...
    private static final Integer[] MOD_11_FACTORS1 = { 5, 4, 3, 2, 7, 6, 5, 4, 3, 2 };
    private static final Integer[] MOD_11_FACTORS2 = { 3, 7, 6, 1, 8, 9, 4, 5, 2 };
    ...
    private static int[] extractDigits(long accountNumber) {
        ...
        for (int i = 0; i < 11; i++) {
            digits[i] = (int) ((accountNumber % (long) Math.pow(10, 11 - i)) / (long) Math.pow(10,
                                                                                               11 - i - 1));
        }
        return digits;
    }

    private static boolean sjekk(final List<Integer> fnr, final List<Integer>fakt, final int sum, final int index) {
        if ( index==fakt.size() ) {
          int mod11= sum % 11;
